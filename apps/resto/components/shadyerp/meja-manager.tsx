...
} from "../../lib/shadyerp/resto-actions";
...
export function getTableDisplayCode(name: string): string {
  if (name.includes("|")) {
    return name.split("|")[1]?.trim() ?? name.trim();
  }
  let code = name.trim();
...
export function TableVisual({
  ...
  if (shape === "RECTANGLE") {
    ...
    const topCount = perSide[0] ?? 0;
    const rightCount = perSide[1] ?? 0;
    const bottomCount = perSide[2] ?? 0;
    const leftCount = perSide[3] ?? 0;

    for (let i = 0; i < topCount; i++) {
      const left = ((tableWidth - chairSize) / Math.max(1, topCount - 1)) * i;
      addChair(`top-${i}`, left, 0, "top");
      plates.push(
        <span
          key={`plate-top-${i}`}
    ...
    for (let i = 0; i < bottomCount; i++) {
      const left = ((tableWidth - chairSize) / Math.max(1, bottomCount - 1)) * i;
      addChair(`bottom-${i}`, left, tableHeight + chairSize + 4, "bottom");
      plates.push(
        <span
          key={`plate-bottom-${i}`}
    ...
    for (let i = 0; i < leftCount; i++) {
      const top = chairSize + 2 + ((tableHeight - chairSize) / Math.max(1, leftCount - 1)) * i;
      addChair(`left-${i}`, 0, top, "left");
      plates.push(
        <span
          key={`plate-left-${i}`}
    ...
    for (let i = 0; i < rightCount; i++) {
      const top = chairSize + 2 + ((tableHeight - chairSize) / Math.max(1, rightCount - 1)) * i;
      addChair(`right-${i}`, tableWidth + chairSize + 4, top, "right");
      plates.push(
        <span
          key={`plate-right-${i}`}
