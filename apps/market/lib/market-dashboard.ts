...
export async function getMarketDashboard({ tenantId, userId, role }: DashboardInput): Promise<MarketDashboard> {
  ...
  return {
    ...
    alerts: alertResult.rows.map((item) => ({ id: item.id, title: `${item.name} stok menipis`, body: `Sisa ${item.qty}; batas minimum ${item.min_qty}.`, href: "/produk" })),
    topProducts: topProductResult.rows.map((item) => ({ name: item.name, quantity: Number(item.quantity), omzet: Number(item.omzet) })),
  };
}
